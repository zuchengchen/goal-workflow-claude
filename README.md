...
- 默认将 goal 文件保存到当前工作目录，文件名为 `<YYYY-MM-DD>-<slug>.md`；除用户显式指定外，不会改放到项目根或其他目录。
- 两次确认后，默认在当前会话按已保存的 goal 文件执行；也可交接——给出可粘贴到新 Claude Code 会话的启动语，稍后再执行。

是否提交 goal 文件由项目决定：个人 goal 通常应在 `.gitignore` 中忽略这类文件名（例如 `/20[0-9][0-9]-[0-9][0-9]-[0-9][0-9]-*.md`），团队共享的 goal 可以显式纳入版本控制。
